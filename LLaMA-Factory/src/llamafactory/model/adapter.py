...
def _setup_freefreeze_tuning(
    model: "PreTrainedModel",
    finetuning_args: "FinetuningArguments",
    is_trainable: bool,
    cast_trainable_params_to_fp32: bool,
) -> None:
    assert finetuning_args.freefreeze_frozen_modules is None or finetuning_args.freefreeze_trainable_modules is None, "Cannot set both frozen and trainable modules."
    assert not (finetuning_args.freefreeze_frozen_modules is None and finetuning_args.freefreeze_trainable_modules is None), "Cannot unset both frozen modules and trainable modules."
    if not is_trainable:
        return

    logger.info("Fine-tuning method: Free Freeze")
    if hasattr(model.config, "text_config"):  # composite models
        config = getattr(model.config, "text_config")
    else:
        config = model.config
    ...
    forbidden_modules = get_forbidden_modules(model.config, finetuning_args)
    if finetuning_args.freefreeze_frozen_modules is not None and finetuning_args.freefreeze_frozen_modules[0] == "all":
        for name, param in model.named_parameters():
            param.requires_grad_(False)
        logger.info("Set untrainable layers: all")
    elif finetuning_args.freefreeze_frozen_modules is not None:
        for name, param in model.named_parameters():
            if any(untrainable_layer in name for untrainable_layer in finetuning_args.freefreeze_frozen_modules) or any(
                forbidden_module in name for forbidden_module in forbidden_modules
            ):
                param.requires_grad_(False)
            else:
        ...
        logger.info("Set untrainable layers: {}".format(",".join(finetuning_args.freefreeze_frozen_modules)))
    elif finetuning_args.freefreeze_trainable_modules is not None:
        for name, param in model.named_parameters():
            if any(trainable_layer in name for trainable_layer in finetuning_args.freefreeze_trainable_modules) and not any(
                forbidden_module in name for forbidden_module in forbidden_modules
            ):
                param.requires_grad_(True)
                if cast_trainable_params_to_fp32:
        ...
        logger.info("Set trainable layers: {}".format(",".join(finetuning_args.freefreeze_trainable_modules)))
    else:
        raise ValueError("No frozen modules or trainable layers specified.")
...
def _setup_lora_tuning(
    ...
    if is_trainable and adapter_to_resume is None:  # create new lora weights while training
        ...
        elif len(finetuning_args.lora_target) == 1 and finetuning_args.lora_target[0] == "model.all":
            target_modules = find_all_linear_modules(model.model, finetuning_args.freeze_vision_tower)
        else:
...
def init_adapter(
    ...
    elif finetuning_args.finetuning_type == "freefreeze":
        _setup_freefreeze_tuning(model, finetuning_args, is_trainable, cast_trainable_params_to_fp32)
